 ...
 * @brief 计算单位球面上的两个正交基
 * 两种形式计算残差
 * 第一种是在归一化平面上的重投影误差，这种误差更常见，也更好理解。
 * 第二种是把归一化平面上的重投影误差投影到Unit Sphere的误差。通过UNIT_SPHERE_ERROR控制
 */
...
{
    TicToc tic_toc;
    Eigen::Vector3d Pi(parameters[0][0], parameters[0][1], parameters[0][2]);                      // 第i帧世界坐标系下的平移
    Eigen::Quaterniond Qi(parameters[0][6], parameters[0][3], parameters[0][4], parameters[0][5]); // 第i帧世界坐标系下的旋转

    Eigen::Vector3d Pj(parameters[1][0], parameters[1][1], parameters[1][2]);                      // 第j帧世界坐标系下的平移
    Eigen::Quaterniond Qj(parameters[1][6], parameters[1][3], parameters[1][4], parameters[1][5]); // 第j帧世界坐标系下的旋转

    Eigen::Vector3d tic(parameters[2][0], parameters[2][1], parameters[2][2]);                      // 第i帧IMU坐标系到第i帧相机坐标系的平移
    Eigen::Quaterniond qic(parameters[2][6], parameters[2][3], parameters[2][4], parameters[2][5]); // 第i帧IMU坐标系到第i帧相机坐标系的旋转

    double inv_dep_i             = parameters[3][0];                  // 第i帧相机坐标系下的的逆深度
    Eigen::Vector3d pts_camera_i = pts_i / inv_dep_i;                 // 第i帧相机坐标系下的3D坐标，pts_i是i时刻归一化相机坐标系下的3D坐标
...
/**
 * @brief 重投影误差计算
 * 残差（residual）是指一个模型或方程预测值与观测值之间的差异。通常，我们用残差来衡量一个模型或方程对数据的拟合程度，残差越小，表示模型或方程对数据的拟合越好。
 * 在非线性优化中，我们也可以通过最小化残差来优化目标函数，例如在相机位姿估计中，我们可以最小化图像中特征点在相机坐标系下的重投影误差（即残差）来求解相机位姿。
 */
#if UNIT_SPHERE_ERROR
