...
void printStatistics(const Estimator &estimator, double t)
{
    ...
    // 这里输出的是最新的一帧的位姿，输出的频率和WINDOW_SIZE有关
    printf("[estimator] position: %f, %f, %f\r", estimator.Ps[WINDOW_SIZE].x(), estimator.Ps[WINDOW_SIZE].y(), estimator.Ps[WINDOW_SIZE].z());
    ROS_DEBUG_STREAM("[estimator] position: " << estimator.Ps[WINDOW_SIZE].transpose());
    ROS_DEBUG_STREAM("[estimator] orientation: " << estimator.Vs[WINDOW_SIZE].transpose());
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ROS_DEBUG("[estimator] calibration result for camera %d", i);
        ROS_DEBUG_STREAM("[estimator] extirnsic tic: " << estimator.tic[i].transpose());
        ROS_DEBUG_STREAM("[estimator] extrinsic ric: " << Utility::R2ypr(estimator.ric[i]).transpose());
        // 根据配置中的定义estimate_extrinsic = 1或者2时
        if (ESTIMATE_EXTRINSIC)
        {
            // EX_CALIB_RESULT_PATH = OUTPUT_PATH + "/extrinsic_parameter.csv";
            cv::FileStorage fs_extrinsic(EX_CALIB_RESULT_PATH, cv::FileStorage::WRITE);
            ...
            fs_extrinsic << "extrinsicRotation" << cv_R << "extrinsicTranslation" << cv_T;
    ...
    sum_of_time += t;
    sum_of_calculation++;
    ROS_DEBUG("[estimator] vo solver costs: %f ms", t);
    ROS_DEBUG("[estimator] vo calculate average time: %f ms", sum_of_time / sum_of_calculation);

    sum_of_path += (estimator.Ps[WINDOW_SIZE] - last_path).norm();
    last_path = estimator.Ps[WINDOW_SIZE];
    ROS_DEBUG("[estimator] sum of path %f", sum_of_path);
    // 根据配置的定义 estimate_td = 1时
    if (ESTIMATE_TD)
    {
        ROS_INFO("[estimator] td %f", estimator.td);
    }
}
...
void pubKeyframe(const Estimator &estimator)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)
    {
        ...
        odometry.pose.pose.orientation.z = R.z();
        odometry.pose.pose.orientation.w = R.w();
        //printf("[estimato] odometry pose timestamp: %f t: (%f, %f, %f) r: (%f, %f, %f, %f)\n", odometry.header.stamp.toSec(), P.x(), P.y(), P.z(), R.x(), R.y(), R.z(), R.w());
