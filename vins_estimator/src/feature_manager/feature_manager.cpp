...
bool FeatureManager::addFeatureCheckParallax(int frame_count, map<int, Eigen::Matrix<double, 7, 1>> &image, double td)
{
    ...
    for (auto iter = image.begin(), iter_next = image.begin(); iter != image.end(); iter = iter_next)
    {
        ...
        unsigned short pt_depth_mm = depth_img.at<unsigned short>((int)iter->second(4), (int)iter->second(3));
        ...
        if (it == feature.end())
        {
            ...
            feature.back().feature_per_frame.emplace_back(FeaturePerFrame(iter->second, td, pt_depth_m));
        }
...
void FeatureManager::removeFailuresOutliers(Vector3d Ps[], Matrix3d _Rs[], Vector3d tic[], Matrix3d _ric[])
{
    for (auto it = feature.begin(), it_next = feature.begin(); it != feature.end(); it = it_next)
    {
        it_next++;
        ...
        else
        {
            ...
            for (auto &it_per_frame : it->feature_per_frame)
            {
                ...
                if (imu_i != imu_j)
                {
                    ...
                    Vector3d pts_cj   = _ric[0].transpose() * (_Rs[imu_j].transpose() * (pts_w - Ps[imu_j]) - tic[0]);
...
void FeatureManager::triangulateWithDepth(Vector3d _Ps[], Vector3d _tic[], Matrix3d _ric[])
{
    for (auto &it_per_id : feature)
    {
        ...
        for (int k = 0; k < (int)it_per_id.feature_per_frame.size(); k++)
        {
            ...
            Eigen::Vector3d point0(it_per_id.feature_per_frame[k].point * it_per_id.feature_per_frame[k].depth);
            ...
            for (int j = 0; j < (int)it_per_id.feature_per_frame.size(); j++)
            {
                ...
                Eigen::Vector2d residual =
                    point1_2d - Vector2d(point1_projected.x() / point1_projected.z(), point1_projected.y() / point1_projected.z());
        ...
        if (verified_depths.empty())
        {
            ...
            {
                if (no_depth_num == (int)it_per_id.feature_per_frame.size())
                {
                    int imu_j = imu_i - 1;
...
void FeatureManager::initFramePoseByPnP(int frameCnt, Vector3d Ps[], Matrix3d Rs[], Vector3d tic[], Matrix3d ric[])
{
                      ...
                      // tzhang
    {
        ...
        {
            if (it_per_id.estimated_depth > 0) //该路标点完成了初始化，使用初始化完成的路标点，获取3D-2D点对
                                               // tzhang
            {
